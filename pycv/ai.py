                        ...
                        try:
                            ...
                            if self.provider.endpoint.endswith('/completions'):
                                ...
                                try:
                                    ...
                                    if response is None:
                                        self.logger.error("Received None response from completions endpoint")
                                        raise ValueError("Received None response from completions endpoint")
                                    ...
                                    if hasattr(response, 'choices') and len(response.choices) > 0:
                                        if hasattr(response.choices[0], 'text'):
                                            ...
                                            self.logger.warning("Response choices doesn't have 'text' attribute")
                                            content = str(response.choices[0])
                                    else:
                                        self.logger.warning("Response doesn't have expected 'choices' structure")
                                    ...
                                    self.logger.error(f"Error with completions endpoint: {e}")
                                    # Return a stub response based on the expected model type
                                    if issubclass(respmodel, Summary):
                                        return Summary(summary="Unable to generate summary with local LLM.")
                                    elif issubclass(respmodel, Letterinfo):
                                        return Letterinfo(
                                            recipient=["Company"],
                                            subject="Application",
                                            opening="Dear Hiring Manager,",
                                            content="Unable to generate content with local LLM."
                                        )
                                    elif hasattr(respmodel, '__origin__') and respmodel.__origin__ is Iterable:
                                        # For Iterable types, return an empty list or a list with a stub item
                                        item_type = respmodel.__args__[0]
                                        if issubclass(item_type, JobDescription):
                                            return [JobDescription(job=1, description="Unable to generate job description with local LLM.")]
                                        elif issubclass(item_type, Cvitem):
                                            return [Cvitem(job=1, item="Unable to generate CV items with local LLM.")]
                                        else:
                                            return []
                                    else:
                                        # Last resort: return a stub instance
